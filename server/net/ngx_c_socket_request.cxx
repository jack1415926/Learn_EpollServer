...
bool CSocekt::IsIpBlacklisted(const std::string& ip) {
    std::lock_guard<std::mutex> lock(m_blacklistMutex); // 加锁保护黑名单访问
    auto it = m_localBlacklist.find(ip);
    if (it != m_localBlacklist.end()) {
        if(time(nullptr) < it->second) {
            return true; // IP在黑名单中且未过期
        } else {
            m_localBlacklist.erase(it); // IP过期，从黑名单中移除
        }
    }
    return false; // IP不在黑名单中
}

void CSocekt::AddIpToBlacklist(const std::string& ip, int expire_seconds) {
    std::lock_guard<std::mutex> lock(m_blacklistMutex); // 加锁保护黑名单访问
    m_localBlacklist[ip] = time(nullptr) + expire_seconds; // 将IP添加到黑名单中，设置过期时间
}
...
{  
    ...
    //新增L1n本地防线：IP极速拦截
    //动作：在调用recv系统函数收包之前，先查明这个IP是否在本地黑名单中
    u_char ip_text[100]={0};
    //提取当前连接的客户端IP（参数0表示不需要附带端口号）
    ngx_sock_ntop(&pConn->s_sockaddr,0,ip_text,sizeof(ip_text));
    std::string client_ip((const char*)ip_text);

    //查询本地哈希表（读锁/互斥锁保护）
    if(IsIpBlacklisted(client_ip))
    {
        //如果在黑名单中，并且还在60秒封禁期内，不执行任何收包操作，直接关闭Socket（底层会自动回复TCP RST/FIN并回收句柄）
        
        // 【注销日志打印】：正式环境/压测环境下，切忌在这里打日志，防磁盘被打满
        //ngx_log_stderr(0,"CSocekt::ngx_read_request_handler()发现IP在本地黑名单中，丢弃该包，IP: %s", client_ip.c_str());
    ...
    else if(pConn->curStat == _PKG_BD_INIT) 
    {
        ...
        if(reco == pConn->irecvlen)
        {
            ...
            //已注释旧版的基于单机的TestFlood调用，统一改为L2业务线程的Redis Lua限流
    ...
    // 因为我们注释掉了旧版 TestFlood，这里的 isflood 永远是 false
    // 恶意用户判定已经推迟到 L2 Redis 去拦截，L2 发现后会写回到上方的 L1 黑名单里
    if(isflood == true)
    {
