...
> 状态日期：2026-09-15。个人项目范围内的最终Debug基线已完成；它用于验证当前修改和提供简历量级，不代表生产环境或服务器理论上限。
...
最终三轮最大QPS与最小QPS相差约24%，且吞吐随客户端和Worker获得的CPU时间同步变化，因此简历只写约5.2万QPS中位数，并明确“12-vCPU VMware本地环回、Debug、Redis限流关闭”；不能写成稳定生产吞吐或服务器上限。
...
多次VM重启已清理早期 `/tmp` 原始文件，早期数据仅保留本文汇总。本次最终干净提交基线的客户端和资源JSON位于 `/tmp/epoll-final-debug.XbAu3a/`；它仍是临时目录，长期证据以本文汇总为准。

## 简历建议口径

> 基于Linux epoll、Master-Worker和线程池实现并发TCP服务器，完善连接生命周期、优雅退出、MySQL连接池和Redis Cache-Aside；重构Python压测工具，支持多进程、完整协议校验、内核超时和资源采样。在12-vCPU VMware本地环回、Debug且Redis限流关闭条件下，200并发Ping三轮中位数约5.2万QPS，600万请求全部成功。
