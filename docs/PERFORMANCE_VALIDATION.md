...
> 状态日期：2026-09-15。个人项目范围内的最终Debug基线及Redis-on对照已完成；它们用于验证当前修改和提供简历量级，不代表生产环境或服务器理论上限。
...
单进程改用内核超时后，严格口径由约0.89W提升到约1.61W，提升约81%。6进程最新长期中位数约5.31W，相比更早的约5.03W高约5.6%，但两批测试跨VM重启且CPU分配不同，不能把这部分增幅全部归因于超时实现；当前个人项目不再用跨批次数字追求额外结论。

- 最终干净提交基线：Git提交 `9072832`、工作树 `git_dirty=false`、Debug、12 vCPU/约3.8 GiB、4 Worker × 120线程、Redis限流关闭；服务端固定CPU 0–3，6个客户端进程固定CPU 4–11，总并发200，每连接10000请求，内核超时、`recv_into`严格校验、1/100延迟抽样。预热后正式三轮QPS为44312、51873、54906，中位数51873；p50/p95/p99中位数为3.00/6.69/9.33 ms。三轮共6000000/6000000成功、失败0、发送丢包0，队列观测峰值不超过收10/发25，单Worker最大RSS约28 MiB。

- 最终Redis-on对照：文档提交 `10065ed`、工作树 `git_dirty=false`，服务端和压测代码仍与 `9072832` 相同；除临时配置启用Redis限流并将阈值提高到100000000外，其余构建、CPU亲和性、并发和客户端参数与Redis-off基线相同。三轮QPS为23846、27893、34728，中位数27893；p50/p95/p99中位数为5.17/10.59/14.02 ms，6000000/6000000成功、失败0、发送丢包0。Redis `commandstats` 在本次重启后记录6100000次 `EVAL`，恰好对应10万预热和600万正式请求，`failed_calls=0`、`rejected_calls=0`；Redis平均CPU约50%–62%。

同口径中位数对比：Redis-on相对Redis-off的QPS由51873降至27893，下降约46.2%；p95由6.69 ms升至10.59 ms，增加约58.3%。这反映当前设计对每个合法业务包同步执行一次Redis Lua往返的成本，不表示Redis服务异常。

Redis-off三轮最大QPS与最小QPS相差约24%，Redis-on跨度更大；吞吐随客户端、Worker和Redis获得的CPU时间同步变化。因此简历使用三轮中位数，并明确“12-vCPU VMware本地环回、Debug”；不能写成稳定生产吞吐或服务器上限。
...
多次VM重启已清理早期 `/tmp` 原始文件，早期数据仅保留本文汇总。Redis-off最终基线JSON位于 `/tmp/epoll-final-debug.XbAu3a/`，Redis-on对照位于 `/tmp/epoll-final-redis-on.2qxi5Z/`；它们仍是临时目录，长期证据以本文汇总为准。

## 简历建议口径

> 基于Linux epoll、Master-Worker和线程池实现并发TCP服务器，完善连接生命周期、优雅退出、MySQL连接池和Redis Cache-Aside；重构Python压测工具，支持多进程、完整协议校验、内核超时和资源采样。在12-vCPU VMware本地环回Debug环境下，200并发Ping三轮中位数约5.2万QPS，600万请求全部成功；开启每请求Redis Lua限流后中位数约2.8万QPS，并据此量化同步Redis往返约46%的吞吐成本。
