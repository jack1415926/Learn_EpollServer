...
> 快照时间：2026-09-14，工作目录 `/home/yzy/epoll`。新会话应先阅读本文，再查看 [性能验证说明](PERFORMANCE_VALIDATION.md) 和 [优化路线图](OPTIMIZATION_AND_AGENT_ROADMAP.md)。
...
历史原始JSON路径如下；多次VM重启已清理除最后一个6/8进程公平对照目录外的其他 `/tmp` 目录，早期数据仅保留本文汇总：
...
内核超时让严格单进程相对约0.89W提升约81%。6进程最新中位数比早期约5.03W高约5.6%，但两批结果跨VM重启且CPU分配不同，不能据此宣称多进程也由同一修复确定提升5.6%；Release阶段将用当前固定配置重新建立可比基线。
...
`testscript/performance_sampler.py` 的实现和离线验证已经完成。正式复测时仍须实际同步记录：
...
采样结果必须和该轮客户端 JSON 使用同一标签和目录；示例命令见 [PERFORMANCE_VALIDATION.md](PERFORMANCE_VALIDATION.md)。

### 第三步：继续历史参数复现

Debug 历史参数、旧版本交叉实验和多进程客户端校准已经完成。接下来应优先把压测端移到另一台机器；若暂时只能同机运行，则沿用6进程、1/100抽样和CPU隔离，并明确记录较大的VM轮间波动，再保持4 Worker × 120线程运行Release候选：

```text
200 并发 × 2000 请求 = 400000 请求
```

Debug 只用于和旧记录粗略比较，Release 才作为当前正式候选。不要修改并提交默认的 `RedisRateLimitEnable = 1`。

### 第四步：并发阶梯和 Redis 对照

对 50、100、200、400 并发分别运行，固定每连接请求数。每档预热一次、正式三次，报告三轮中位数而非最好值。
...
客户端和服务端目前同处12 vCPU VM。单进程减半已定位为Python用户态超时socket开销，默认已改为Linux内核收发超时；6进程在相同4/8核分配下的长期中位数约5.31W，高于8进程约4.94W。当前剩余问题是同机轮间仍有约7%–13%波动；应优先改用外部压测机，不能把VMware调度或loopback上限描述成服务器上限。
